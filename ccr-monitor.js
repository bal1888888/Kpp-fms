...
(function (root) {
  ...
  function selectionIssue(row, now = new Date()) {
    ...
    if (row.status !== "READY" || number(row.hm_actual) === null || !row.hm_actual_at || !siteParts(row.hm_actual_at)) return "Menunggu operator mengirim HM aktual. Pilih setelah status READY.";
    if (row.fuel_history_id != null) return "HM operator sudah dipakai untuk pengisian.";
    if ((row.allocations || []).some(a => BUSY.has(a.status) || a.status === "USED")) return "Unit ini masih punya jatah ACTIVE/PENDING_GL. Selesaikan jatah tersebut dahulu.";
    return "";
  }
  ...
  const CHECKIN_HEADERS = ["ID Check-in","Tanggal Operasional","Shift","Jam Check-in","Unit","EGI","Operator","NRP","HM Awal Operator","HM Aktual Operator","Waktu HM Aktual (WIB)","Diterima Server (WIB)","Status Check-in","Jumlah Jatah Terkait","ID Pengisian Check-in","Waktu Pengisian Check-in (WIB)","Kedaluwarsa (WIB)","Alasan Kedaluwarsa"];
  const ALLOCATION_HEADERS = ["ID Jatah","ID Check-in","Tanggal Operasional","Shift","Unit","Operator","NRP","Ritasi","Status Jatah","HM Referensi Saat Jatah","Sumber Referensi HM","HM Aktual Jatah (Operator / Petugas)","Jam HM CCR","Qty Jatah (L)","Toleransi (L)","HM Aktual Saat Jatah Dipakai","Waktu Jatah Dipakai (WIB)","ID Pengisian Jatah","Catatan"];
  ...
  function create({db, host, onSelect}) {
    ...
    <p class="input-help">Waktu site: WIB. HM awal adalah referensi check-in; HM aktual operator dipakai untuk jatah dan pengisian. Filter laporan tidak mengubah form jatah. Unit tanpa operator tidak muncul di daftar check-in.</p>
    ...
    async function download() {
      ...
      try {
        ...
        info.addRows([["KPP-FMS — Monitoring CCR"],["Tanggal awal",filters.from],["Tanggal akhir",filters.to],["Shift",filters.shift || "Semua Shift"],["Pencarian",el("monitorSearch").value],["Status check-in",el("monitorStatus").value || "Semua Status"],["Data diambil (WIB)",timestamp(loadedAt)],["Cakupan","Semua hasil filter, bukan hanya halaman tabel yang sedang terlihat."],["Sumber","operator_unit_checkins dan ccr_allocations; mengikuti izin akun login."],["HM","HM awal adalah referensi check-in. HM aktual jatah berasal dari operator, atau petugas untuk unit tanpa operator."],["Jatah","Hanya jatah yang tertaut lewat ID check-in dan cocok unit/tanggal/shift."],["Batasan","Tidak memuat seluruh unit master atau Qty aktual/Fuelman dari log pengisian."],["Waktu","Tanggal adalah tanggal operasional shift; timestamp ditampilkan dalam WIB."],["Catatan","Data dimuat bertahap, bukan snapshot transaksi database atomik."]]);
