...
test('Stock Closing RPC is SECURITY DEFINER after direct table writes are revoked', () => {
  assert.match(leastPrivilege, /revoke\s+insert\s*,\s*update\s*,\s*delete\s+on\s+table\s+public\.stock_closing\s+from\s+authenticated/i);
  assert.match(leastPrivilege, /revoke\s+insert\s*,\s*update\s*,\s*delete\s+on\s+table\s+public\.stock_opening\s+from\s+authenticated/i);
  assert.match(leastPrivilege, /revoke\s+truncate\s*,\s*references\s*,\s*trigger\s+on\s+all\s+tables\s+in\s+schema\s+public\s+from\s+authenticated/i);
...
test('privilege repair keeps the guarded atomic implementation unchanged', () => {
  assert.match(atomic, /set\s+search_path\s+(?:=|to)\s*''/i);
  ...
  assert.match(atomic, /Sesi Fuelman tidak cocok/i);
});
