-- HM operator is authoritative; no existing transaction rows are rewritten.
-- Site shift boundary is enforced at use time, independently of the cleanup cron.
CREATE OR REPLACE FUNCTION private.kpp_shift_is_current(p_date date, p_shift text)
RETURNS boolean LANGUAGE sql STABLE SECURITY INVOKER SET search_path = '' AS $$
  select coalesce(timezone('Asia/Jakarta', now()) >= public.ccr_shift_end_local(p_date,p_shift) - interval '12 hours'
    and timezone('Asia/Jakarta', now()) < public.ccr_shift_end_local(p_date,p_shift), false);
$$;
REVOKE ALL ON FUNCTION private.kpp_shift_is_current(date,text) FROM PUBLIC;
...
begin
  ...
  if exists (select 1 from public.operator_unit_checkins c
    where c.unit=v_unit and c.tanggal=v_date and c.shift=v_shift and c.status in ('ACTIVE','READY')) then
    raise exception 'Unit masih memiliki check-in aktif. Buka check-in tersebut untuk mengirim HM atau menyelesaikan pengisian.';
  end if;
...
begin
  ...
  if p_actual_hm::text in ('NaN','Infinity','-Infinity') then
    raise exception 'HM aktual tidak valid.';
  end if;
...
begin
  if auth.uid() is null or coalesce(public.current_staff_role(),'') not in ('gl','ccr') then
  ...
  update public.ccr_allocations a set status='EXPIRED', updated_at=now()
    where upper(trim(a.unit))=upper(trim(new.unit)) and a.status in ('ACTIVE','PENDING_GL')
  ...
  select coalesce(u.operator_checkin_required,true) into v_requires_operator
    from public.unit_master u where upper(trim(u.code_unit))=upper(trim(new.unit)) and coalesce(u.active,true);
  if not found then raise exception 'Unit tidak aktif atau tidak ditemukan.'; end if;
  if v_requires_operator then
    select * into v_checkin from public.operator_unit_checkins c
      where c.id=new.operator_checkin_id for update;
    if not found or v_checkin.status <> 'READY' or v_checkin.hm_actual is null
      or v_checkin.fuel_history_id is not null
      or upper(trim(v_checkin.unit))<>upper(trim(new.unit))
      or v_checkin.tanggal is distinct from new.tanggal or v_checkin.shift is distinct from new.shift then
      raise exception 'Pilih check-in READY yang sesuai unit/tanggal/shift. HM aktual operator wajib tersedia.';
    end if;
    if exists(select 1 from public.ccr_allocations a where a.operator_checkin_id=v_checkin.id
      and a.status in ('ACTIVE','PENDING_GL','USED')) then
    ...
    new.hm_ccr := v_checkin.hm_actual;
    new.hm_taken_time := timezone('Asia/Jakarta',v_checkin.hm_actual_at)::time;
  else
    if new.operator_checkin_id is not null then raise exception 'Unit tanpa operator tidak memakai check-in.'; end if;
  ...
  new.filling_no := null; -- Server assigns ritasi, never the client.
...
begin
  ...
  -- Changing quota/approval must never change the identity or HM snapshot.
  if new.operator_checkin_id is distinct from old.operator_checkin_id then
    raise exception 'Tautan check-in jatah tidak boleh diubah.';
  end if;
  if old.operator_checkin_id is not null and
    (row(new.unit,new.tanggal,new.shift,new.operator_unit,new.nrp,new.hm_ccr,new.hm_taken_time,
         new.operator_hm_awal,new.operator_checkin_at)
     is distinct from row(old.unit,old.tanggal,old.shift,old.operator_unit,old.nrp,old.hm_ccr,old.hm_taken_time,
         old.operator_hm_awal,old.operator_checkin_at)) then
    raise exception 'Identitas dan HM operator pada jatah terkunci. Batalkan jatah bila perlu koreksi.';
  end if;
...
begin
  ...
  if auth.uid() is null or coalesce(v_role,'') not in ('gl','admin','fuelman') then
  ...
  select coalesce(u.operator_checkin_required,true) into v_requires_operator from public.unit_master u
    where upper(trim(u.code_unit))=upper(trim(v_alloc.unit)) and coalesce(u.active,true);
  if not found then raise exception 'Unit tidak aktif atau tidak ditemukan.'; end if;
  ...
  -- Historical column names are retained for compatibility; no HM estimation.
  v_estimated_hm := coalesce(v_checkin.hm_actual,v_alloc.hm_ccr);
  v_elapsed_hours := 0;
  if v_estimated_hm is null or v_estimated_hm < 0 or v_estimated_hm::text in ('NaN','Infinity','-Infinity') then
    raise exception 'HM aktual tidak valid.';
  end if;

  v_previous_hm := coalesce(v_alloc.hm_previous_ref, p_previous_hm);

  if v_previous_hm is not null and v_estimated_hm < v_previous_hm then
    raise exception 'HM aktual lebih kecil dari HM referensi. Periksa HM sebelum pengisian.';
  end if;
  ...
  set
    ...
    used_actual_hm = v_estimated_hm,
    used_estimated_hm = null,
    used_estimated_runtime = null
  where id = v_alloc.id;

  if v_checkin.id is not null then
    update public.operator_unit_checkins set status='FUELED', fueled_at=now(), fuel_history_id=v_history.id, updated_at=now()
      where id=v_checkin.id;
  end if;
  return jsonb_build_object(
    'checkin_id', v_checkin.id,
    'hm_actual', v_estimated_hm,
    'ok', true,
...
begin
  ...
  if auth.uid() is null or coalesce(v_role,'') not in ('admin', 'gl', 'fuelman') then
    raise exception
  ...
  if not private.kpp_shift_is_current(v_checkin.tanggal,v_checkin.shift) then
    raise exception 'Check-in bukan milik shift aktif WIB.';
  end if;
  if v_checkin.fuel_history_id is not null then raise exception 'HM operator sudah dipakai.'; end if;
  if not exists(select 1 from public.unit_master u where upper(trim(u.code_unit))=upper(trim(v_checkin.unit)) and coalesce(u.active,true)) then
